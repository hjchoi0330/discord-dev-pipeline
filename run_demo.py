#!/usr/bin/env python3
"""Pipeline demo runner - simulates Claude CLI calls."""
...
# ── 2026-02-28 simulated responses (existing) ───────────────────────────
...
# ── 2026-03-01 simulated responses (Golang multiplication table) ─────────
...
def _mock_call_claude_analyzer(prompt: str, timeout: int = 120) -> str:
    """Simulated response for the analyzer - returns empty topics if no dev-related keywords."""
    prompt_lower = prompt.lower()
    if not any(kw.lower() in prompt_lower for kw in _DEV_KEYWORDS):
        return '{"dev_topics": []}'
    # Return 0301 response if multiplication-table/Go keywords are present
    go_keywords = ["구구단", "golang", "온보딩", "go로"]
    if any(kw in prompt_lower for kw in go_keywords):
        return _MOCK_ANALYSIS_RESPONSE_0301
    # Otherwise return 0228 response for other dev keywords
    return _MOCK_ANALYSIS_RESPONSE_0228


def _mock_call_claude_planner(prompt: str, timeout: int = 180) -> str:
    """Simulated response for the planner - matches topic title from the Title field."""
...
def _mock_executor_for_gugudan(prompt: str, timeout: int = 300) -> str:
    """Simulated executor - actually creates Go files when given a multiplication-table plan."""
...
if __name__ == "__main__":
    ...
    # If a multiplication-table plan was generated, run the executor simulation
    gugudan_plan = Path("data/plans/2026-03-01_go-구구단-cli-프로그램-개발.md")
    if gugudan_plan.exists():
        print("\n[executor simulation] Running multiplication-table plan...")
        result = _mock_executor_for_gugudan("")
        print(f"[executor simulation] {result}")
